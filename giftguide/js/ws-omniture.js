...
/**
* Handles Omniture tracking.
* DO NOT ALTER ANYTHING IN THIS FILE.
* @class omniture
* @namespace Postmedia
*/

// TODO ADD STORY TO CONFIG
// <form id="storyConfig">
//        <input type="hidden" name="prop6" value="Canadian Press/FREE">
//        <input type="hidden" name="prop7" value="Canadian Press">
//        <input type="hidden" name="prop8" value="6727548">
//        <input type="hidden" name="prop36" value="Eaton Centre">
//        <input type="hidden" name="prop48" value=", Canadian Press">
//   </form>

Postmedia.Omniture = (function ($) {

    /**
    * Initializes Omniture tracking.
    * @method init
    * @constructor init
    */
    function Init() {
    ...
    /**
    * Sets the content type (page).
    * @return {String} - the name of the html page
    ...
    */
    ...
    function GetAuthor(){
        ...
        if (Postmedia.adConfig.keys.author) {
            author = Postmedia.adConfig.keys.author;
        }
    ...
    function GetPage(){
        ...
        if (Postmedia.adConfig.keys.page) {
            page = Postmedia.adConfig.keys.page;
        }
    ...
    function GetAdSite(){
        ...
        if(Postmedia.Properties.isMobile){
            ...
            adSite = Postmedia.adConfig.msite;

        } else {

            adSite = Postmedia.adConfig.site;
    ...
    function GetPostId(){
        ...
        if (Postmedia.adConfig.keys.aid) {
            postId = Postmedia.adConfig.keys.aid;
        }
    ...
    /**
    * Splits the location pathname of the URI into an array.
    * @return {Array}
    * @method getLocation
    */
    function GetLocation() {

        var fullUri = window.location.pathname.split('#');
        var locationArr = '';

        if (fullUri.length > 0) {
            /* Because of JQM: Get last hash location */
            locationArr = fullUri[fullUri.length - 1].split("/");
        } else {
            locationArr = fullUri.split("/");
        }

        /*remove html page*/
        locationArr.pop();
        /*the fullUri part is a hack becase of redirect from /mobile/ */
        if (SetContentType() == 'story' || SetContentType() == 'comments' && fullUri != '/touch/story.html') {
            /*remove last 2 folders*/
            locationArr.pop();
            locationArr.pop();
        }
    ...
    /**
    * Sets all the Omniture tracking variables for the page.
    * Triggers the Omniture recording event.
    * If on anything but production, will spit out the data to the console.
    * @method Track
    */
    function Track() {

        var aPath = window.location.pathname.toLowerCase().replace(/^\/(.+)(?:\/(?:[^\/.]+\.[\w\d]+)?)$/, '$1').split('/');

        s.pageName = SetPageName();
        s.server = window.location.hostname.toLowerCase();
        s.channel = SetChannel();
        s.pageType = '';
        s.prop1 = (GetPage() == 'index' ) ? '' : aPath.slice(0, 2).join('/');
        s.prop2 = (GetPage() == 'index' ) ? '' : aPath.join('/');
        s.prop3 = 'canwest';
        s.prop4 = 'Non-Registered';
        s.prop5 = s.prop4 + ': ' + s.pageName;
        s.prop6 = ''; /*Story ConditionalAccess  ie. s.prop6='Reuters/FREE';*/
        s.prop7 = ''; /*Story Distributor ie. s.prop7='Reuters';;*/
        s.prop8 = GetPostId();
        s.prop9 = '';
        s.prop10 = '';
        s.prop11 = '';
        ...
        if (Postmedia.Utils.isProduction == false) {
            /*can remove this once we sure it's reporting properly*/
            Log('s_account= ' + s_account);
            Log('s.pageName= ' + s.pageName);
            Log('s.server= ' + s.server);
            Log('s.channel= ' + s.channel);
            Log('s.pageType= ' + s.pageType);

            //for (var j = 1; j <= 50; j++) {
            //    if (new Function("return s.prop" + j)()) {
            //        Log("s.prop" + j + "= " + new Function("return s.prop" + j)())
            //    }
            //}
        }
    ...
    function SetChannel(){
        ...
        var channel = "";
         ...
         if (Postmedia.adConfig.keys.ck) {
            channel = Postmedia.adConfig.keys.ck;
         }
    ...
    function SetTopic() {
        ...
        //IE s.prop36='Entertainment|IAB|100~Billy Zane||100~Kate

        var prop36 = '';

        if (Postmedia.Tracking) {

            if(Postmedia.Tracking.iabs){
                $.each(Postmedia.Tracking.iabs, function (key, value) {

                    prop36 += value.label + "|";
                    prop36 += "IAB";
                    prop36 += "|" + value.rel;

                    if (key != Postmedia.Tracking.iabs.length - 1 || Postmedia.Tracking.tags) {
                        prop36 += "~";
                    }
                });
            }

            if(Postmedia.Tracking.tags){
                $.each(Postmedia.Tracking.tags, function (key, value) {

                    prop36 += value.label + "|";
                    prop36 += value.rel;

                    if (key != Postmedia.Tracking.tags.length - 1) {
                        prop36 += "~";
                    }
                });
            }
    ...
    /**
    * Sets the PostMedia specific PageName data
    * @return {String} - pageName
    * @method setPageName
    */
    function SetPageName() {

        var pageName = window.location.pathname.toLowerCase() + GetPage() + ".html(WP)";
        return pageName;
    }

    function SetLinkTracking() {

        //Hook up Menu click listeners
        $("select.postform[id$=cat]").on("change", { msg: "Discussion"}, LinkTrack);

    }

    /**
    * Sets up the Omniture click event.
    * @param {obj} event - the event
    * @method LinkTrack
    */
    function LinkTrack(event) {

        if(event.target.options){ //is DropDown Box
            FireLinkTrack(event.data.msg + " - " + event.target.options[event.target.options.selectedIndex].text);
        }

    }

    /**
    * Fires the Omniture click event.
    * @param {string} trackMsg - the event you want to record
    * @method FireLinkTrack
    */
    function FireLinkTrack(trackMsg){

        var s = s_gi(s_account);
        s.trackExternalLinks = false;
        s.linkTrackVars = 'prop38';
        s.linkTrackEvents = s.events = 'click';
        s.prop38 = trackMsg + "(WP)";
        s.tl(this, 'o', trackMsg);

        Log("LinkTrack::" + trackMsg);

    }

     /**
    * Triggers an Omniture event page refresh.
    * @param {String} eventText - the event you want to record
    * @method statsEvent
    */
    function StatsEvent(eventText) {
        /*omniture event*/
        Log("StatsEvent::" + eventText);
        s.prop38 = s.pageName + eventText + "(WP)";
        s.events = '';
        void (s.t());
    }
	...
	/**
	 * Namespaced Omniture Tracking Code
	 */
	...
	var s_account = "canwestglobaldev,canpublishing"
	var s = s_gi(s_account)
	...
	s.linkInternalFilters = "javascript:,expedia.ca,maxvr,medbroadcast,publicationsunbound,yellowpages.ca,dreammates,canwest,canada.com"
	...
	s.trackingServer = "canwest.112.2o7.net"
...
jQuery(document).ready(function () { Postmedia.Omniture.Init(); });

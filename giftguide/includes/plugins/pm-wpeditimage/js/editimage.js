...
wpImage = {
	...
	setup : function() {
		...
		// set image credit from data field
		img_credit = ed.dom.getAttrib(el, 'data-credit');
				
		// check if caption begins with our new credit field and remove
		if(caption.trim().substr(0, 6) == 'PHOTO:') {
			// update img_credit from caption... incase someone edited the field directly it could be out of sync from data field
			img_credit = (caption.indexOf('>') == -1) ? caption.substr(6).trim() : caption.substr(6, caption.indexOf('<') - 6).trim();
			
			caption = (caption.indexOf('>') == -1) ? '' : caption.substr(caption.indexOf('>') + 1);
		}
	...
	update : function() {
		...
		if ( ( f.img_cap_text.value != '' || f.img_credit.value != '' ) && f.width.value != '' ) {
			do_caption = 1;
		...
		// PM rules for alt - Chris Murphy
		alt_text = f.img_alt.value;
		if(alt_text == '') alt_text = f.img_cap_text.value;
		if(alt_text == '') alt_text = f.img_title.value;
		if(alt_text == '') alt_text = jQuery('#title').val();
		...
		ed.dom.setAttribs(el, {
			...
			'class' : img_class,
			'data-credit' : f.img_credit.value
		});
		...
		if ( do_caption ) {
			...
			credit = (f.img_credit.value != '') ? 'PHOTO: ' + f.img_credit.value + '<br/>' : '';
			...
			if ( DL ) {
				...
				if ( (DT = ed.dom.getParent(el, 'dt')) && (DD = DT.nextSibling) && ed.dom.hasClass(DD, 'wp-caption-dd') )
					ed.dom.setHTML(DD, credit + caption);
				...
				'px"><dt class="wp-caption-dt">'+html+'</dt><dd class="wp-caption-dd">'+credit + caption+'</dd></dl>';
